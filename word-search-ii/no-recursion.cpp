...
std::vector<std::string> find_words(std::vector<std::vector<char>>& board, std::vector<std::string>& words) {
    ...
    const auto consume_stack = [&]() {
        do {
            auto [r, c, node] = stack.back();

            if (!is_lower(board[r][c])) {
                // Unmark this square.
            ...
            node = (*node)[board[r][c]];
            if (node == nullptr) {
                // We're no longer a prefix of some word.
                stack.pop_back();
                continue;
            }
            if (node->match) {
            ...
            if (r > 0 && is_lower(board[r - 1][c])) {
                stack.push_back(Frame{r - 1, c, node});
            }
            // below:
            ...
            if (c > 0 && is_lower(board[r][c - 1])) {
                stack.push_back(Frame{r, c - 1, node});
            }
            // rightward:
    ...
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            ...
            stack.push_back(Frame{r, c, trie.get()});
            consume_stack();
        }
    }
    return found;
